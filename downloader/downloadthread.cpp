...
void CleanUpDownload( RequestContext& rc, HTTPStatus status, HTTPError error );
...
void __stdcall DownloadStatusCallback( HINTERNET hOpenResource, DWORD dwContext, DWORD dwStatus, LPVOID pStatusInfo, DWORD dwStatusInfoLength )
{
	...
	switch (dwStatus)
	{
		...
		case INTERNET_STATUS_CONNECTION_CLOSED:
			...
			Con_DPrintf( "DownloadStatusCallback %s\n", StateString(dwStatus) );
			break;
	}
}
...
void ReadData( RequestContext& rc )
{
	...
	if ( !rc.nBytesTotal )
	{
		...
		Con_Printf("File is not available via HTTP! disconnecting.");
		return;
	}
	CL_SetFileDownloadPercent(0);
	...
	while ( !rc.shouldStop )
	{
		...
		if ( !InternetReadFile( rc.hDataResource, (LPVOID)data, BufferSize, &dwSize ) )
		{
			...
			CleanUpDownload(rc, HTTP_ERROR,HTTP_ERROR_CONNECTION_CLOSED);
		...
		//	Con_Printf("\n\n\n%s\n\n\n",((rc.nBytesTotal * rc.nBytesCurrent)/100));
			CL_SetFileDownloadPercent((rc.nBytesTotal * rc.nBytesCurrent)/100);
...
void CleanUpDownload( RequestContext& rc, HTTPStatus status, HTTPError error = HTTP_ERROR_NONE )
{
	...
	CL_SetFileDownloadPercent(0);
	CL_UpdateProgressBar();
	if (error != HTTP_ERROR_NONE && strcmp(".bsp",rc.gamePath) == 1)
	{
			MSG_WriteByte(net_message, 3);
			MSG_WriteString(net_message, (int)va("dlfile %s", rc.gamePath));
	}
 ...
 */
unsigned int __stdcall DownloadThread( void *voidPtr )
{
	...
	*/
	...
	rc.hOpenResource = InternetOpen( "Half-Life", INTERNET_OPEN_TYPE_PRECONFIG ,NULL, NULL, 0);
	...
	//	Con_DPrintf( "Requesting partial download\n%s", headers );
	}
	else if ( *rc.serverURL )
	{
		Q_snprintf( headers, BufferSize, "Referer: hl://%s\n", rc.serverURL );
		headerPtr = headers;
		headerLen = (DWORD)-1L; // the DWORD cast is because we get a signed/unsigned mismatch even with an L on the -1.
	//	Con_DPrintf( "Requesting full download\n%s", headers );
	}
